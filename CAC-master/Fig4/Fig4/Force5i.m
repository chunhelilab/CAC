...
out(1,:)=g_Z.*Hs(snail,snail0_sZ,lambda_sZ,n_sZ).*TGF.*Hs(Z,Z0_ZZ,lambda_ZZ,n_ZZ)-k_Z.*Z.*Hs(miR200,miR2000_m2Z,lambda_m2Z,n_m2Z).*Hs(miR145,miR1450_m1Z,lambda_m1Z,n_m1Z);
out(2,:)=g_o.*Hs(p53,p0_po,lambda_po,n_po).*Hs(lin,lin0_lo,lambda_lo,n_lo)-k_o.*oct4.*Hs(miR145,miR1450_m1o,lambda_m1o,n_m1o).*Hs(oct4,oct40_oo,lambda_oo,n_oo);
out(3,:)=g_M.*Hs(p53,p0_pM,lambda_pM,n_pM)-k_M.*MD.*Hs(miR145,miR1450_m1M,lambda_m1M,n_m1M);
out(4,:)=g_s.*Hs(snail,snail0_ss,lambda_ss,n_ss).*TGF-k_s.*snail.*Hs(miR34,miR340_m3s,lambda_m3s,n_m3s).*Hs(let,let0_ls,lambda_ls,n_ls).*Hs(rkip,rkip0_rs,lambda_rs,n_rs)./8;
out(5,:)=g_m1.*Hs(p53,p0_pm1,lambda_pm1,n_pm1)-k_m1.*miR145.*Hs(Z,Z0_Zm1,lambda_Zm1,n_Zm1).*Hs(oct4,oct40_om1,lambda_om1,n_om1);
out(6,:)=g_m2.*Hs(p53,p0_pm2,lambda_pm2,n_pm2).*Hs(oct4,oct40_om2,lambda_om2,n_om2)-k_m2.*miR200.*Hs(Z,Z0_Zm2,lambda_Zm2,n_Zm2).*Hs(snail,snail0_sm2,lambda_sm2,n_sm2);
out(7,:)=g_m3.*Hs(p53,p0_pm3,lambda_pm3,n_pm3)-k_m3.*miR34.*Hs(Z,Z0_Zm3,lambda_Zm3,n_Zm3).*Hs(snail,snail0_sm3,lambda_sm3,n_sm3);
out(8,:)=g_p.*P530-k_p.*p53.*Hs(MD,MD0_Mp,lambda_Mp,n_Mp);
out(9,:)=g_rk.*9.*Hs(bach,bach0_br,lambda_br,n_br)-k_rk.*rkip.*Hs(snail,snail0_sr,lambda_sr,n_sr)./5;
out(10,:)=g_let.*Hs(miR200,miR2000_m2le,lambda_m2le,n_m2le).*Hs(rkip,rkip0_rl,lambda_rl,n_rl)-k_let.*let.*Hs(lin,lin0_llet,lambda_llet,n_llet).*Hs(let,let0_let,lambda_let,n_let);
out(11,:)=g_lin.*Hs(MEK,MEK0_Ml,lambda_Ml,n_Ml).*Hs(oct4,oct40_ol,lambda_ol,n_ol).*Hs(rkip,rkip0_rli,lambda_rli,n_rli)-k_lin.*lin.*Hs(let,let0_ll,lambda_llin,n_llin).*Hs(lin,lin0_lin,lambda_lin,n_lin).*Hs(miR200,miR2000_m2li,lambda_m2li,n_m2li);
out(12,:)=g_b.*10.*Hs(bach,bach0_bb,lambda_bb,n_bb)-k_b.*bach.*Hs(let,let0_lb,lambda_lb,n_lb)./5;
out(13,:)=g_MEK.*TGF.*Hs(MEK,MEK0_MM,lambda_MM,n_MM)-k_MEK.*MEK.*Hs(rkip,rkip0_rM,lambda_rM,n_rM).*MEKi.*Hs(ERK,ERK0_EM,lambda_EM,n_EM);
out(14,:)=g_ERK.*Hs(MEK,MEK0_ME,lambda_ME,n_ME).*Hs(ERK,ERK0_EE,lambda_EE,n_EE)-k_ERK.*ERK.*Rosi.*Hs(ERK,ERK0_EEi,lambda_EEi,n_EEi);
out(15,:)=g_CEBP.*Rosi.*Hs(MEK,MEK_MC,lambda_MC,n_MC).*Hs(PPAR,PPAR_PC,lambda_PC,n_PC)-k_CEBP.*Hs(ERK,ERK0_EC,lambda_EC,n_EC).*CEBP.*TGF;
out(16,:)=g_PPAR.*Rosi.*Hs(MEK,MEK_MP,lambda_MP,n_MP).*Hs(CEBP,CEBP0_CP,lambda_CP,n_CP)-k_PPAR.*Hs(ERK,ERK0_EP,lambda_EP,n_EP).*PPAR;
